...
class Movimientos(models.Model):
    ...
    fecha_cambio = fields.Date(string='Fecha de cambio de ubicación', required=True, default=fields.Date.context_today)
    departamento_id = fields.Many2one('materiales.departamento', string='Departamento', required=True)
...
class MovimientosDetalle(models.Model):
    ...
    departamento_origen_id = fields.Many2one('materiales.departamento', string='Departamento origen')
    edificio_origen = fields.Char(string='Edificio origen')
    area_origen = fields.Char(string='Área origen')
    departamento_destino_id = fields.Many2one('materiales.departamento', string='Departamento destino')
    ...
    def _onchange_producto_id(self):
        if self.producto_id:
            ...
            self.numero_serie = self.producto_id.serial_number
            self.descripcion = self.producto_id.description
        else:
            self.nombre = ''
            self.numero_serie = ''
            self.descripcion = ''
